...
const inventarioKardexController = {
  ...
  // Registrar compra a proveedor (entrada con costo)
  registrarCompra: async (req, res) => {
    ...
    try {
      const {
        ...
        numero_factura,
        fecha_factura,
        productos, // Array: [{ producto_id, cantidad, costo_unitario, precio_venta? }]
        observaciones
      } = req.body;
      ...
      const ultimaCompra = await Compra.findOne({
        where: {
          numero_compra: { [Op.like]: `COM-${anio}-%` }
        },
      ...
      let contador = 1;
      if (ultimaCompra) {
        const numero = parseInt(ultimaCompra.numero_compra.split('-')[2]);
        contador = numero + 1;
      }
