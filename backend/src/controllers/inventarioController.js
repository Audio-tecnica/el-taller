const { Producto, MovimientoInventario, Local, Categoria } = require('../models');
...
const inventarioController = {
  ...
  getInventarioConsolidado: async (req, res) => {
    try {
      ...
      const resumen = {
        ...
        total_stock_local1: productos.reduce((sum, p) => sum + (p.stock_local1 || 0), 0),
        total_stock_local2: productos.reduce((sum, p) => sum + (p.stock_local2 || 0), 0),
        productos_stock_bajo: productos.filter(p => 
          (p.stock_local1 + p.stock_local2) <= p.alerta_stock
        ).length,
        valor_inventario_local1: productos.reduce((sum, p) => 
          sum + ((p.stock_local1 || 0) * parseFloat(p.precio_venta || 0)), 0
        ),
        valor_inventario_local2: productos.reduce((sum, p) => 
          sum + ((p.stock_local2 || 0) * parseFloat(p.precio_venta || 0)), 0
        )
      };
  ...
  getInventarioPorLocal: async (req, res) => {
    try {
      ...
      const productosConStock = productos.map(p => ({
        ...
        stock_bajo: (local_id === '1' ? p.stock_local1 : p.stock_local2) <= p.alerta_stock
      }));
  ...
  getStockBajo: async (req, res) => {
    try {
      ...
      const productosBajos = productos.filter(p => {
        ...
        diferencia: p.alerta_stock - ((p.stock_local1 || 0) + (p.stock_local2 || 0))
      }));
  ...
  ajustarInventario: async (req, res) => {
    ...
    try {
      ...
      await MovimientoInventario.create({
        producto_id,
        local_id: local === 1 ? '00000000-0000-0000-0000-000000000001' : '00000000-0000-0000-0000-000000000002',
        tipo: 'ajuste',
        cantidad: diferencia,
  ...
  getMovimientos: async (req, res) => {
    try {
      const { producto_id, local_id, tipo, fecha_inicio, fecha_fin, limit = 50 } = req.query;
