...
const getPresentacionBadge = (presentacion) => {
  ...
  const c = configs[presentacion] || { bg: 'bg-gray-100', text: 'text-gray-700', icon: '📦' };
  return (
...
export default function FormularioVentaB2B({ onClose, onGuardar }) {
    ...
    return productos
      .filter(p => {
        const matchBuscar =
          ...
          (p.presentacion && p.presentacion.toLowerCase().includes(buscandoProducto.toLowerCase())) ||
          (p.categoria?.nombre && p.categoria.nombre.toLowerCase().includes(buscandoProducto.toLowerCase()));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.cliente_b2b_id) { alert('Por favor seleccione un cliente'); return; }
    if (items.length === 0) { alert('Por favor agregue al menos un producto'); return; }

    try {
      setGuardando(true);
      const user = authService.getCurrentUser();
      await ventasB2BService.crearVenta({
      ...
      >
        ...
        <div className="sticky top-0 bg-gradient-to-r from-gray-800 to-gray-900 text-white px-6 py-5 flex items-center justify-between z-10 rounded-t-xl">
          <div className="flex items-center gap-3">
            <svg className="w-7 h-7 text-[#D4B896]" fill="currentColor" viewBox="0 0 20 20">
          ...
          <button onClick={onClose} className="text-gray-300 hover:text-white text-2xl font-bold transition">×</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <section>
            <h3 className="text-base font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span className="w-6 h-6 bg-[#D4B896] text-gray-900 rounded-full flex items-center justify-center text-xs font-bold">1</span>
              Cliente
            </h3>
              ...
              <div className="mt-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                  <div>
                    ...
                    <div className="font-bold text-gray-800">{formatearMoneda(clienteSeleccionado.limite_credito)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Disponible</div>
                    <div className="font-bold text-green-600">{formatearMoneda(clienteSeleccionado.credito_disponible)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Días Crédito</div>
                    <div className="font-bold text-gray-800">{clienteSeleccionado.dias_credito} días</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Descuento</div>
                    <div className="font-bold text-amber-600">{clienteSeleccionado.descuento_porcentaje}%</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <section>
            <h3 className="text-base font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span className="w-6 h-6 bg-[#D4B896] text-gray-900 rounded-full flex items-center justify-center text-xs font-bold">2</span>
              Productos
            </h3>
          ...
          <section className="bg-gray-50 border-2 border-gray-200 rounded-xl p-5">
            <h3 className="text-base font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span className="w-6 h-6 bg-[#D4B896] text-gray-900 rounded-full flex items-center justify-center text-xs font-bold">3</span>
              Resumen
            </h3>
            <div className="flex justify-end">
              <div className="w-72 space-y-2">
                ...
                <div className="border-t-2 border-gray-300 pt-2 flex justify-between text-xl font-bold text-gray-900">
                  <span>Total</span>
                  <span className="text-emerald-700">{formatearMoneda(total)}</span>
                </div>
              </div>
            </div>
          </section>
          ...
          {/* ── Notas ────────────────────────────────────────────── */}
          <section>
            <label className="block text-sm font-medium text-gray-600 mb-1">Notas / Observaciones</label>
            <textarea
